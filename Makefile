...
test-container: $(BUILDFILE) $(RESULTS_DIR)
	$(CONTAINER_CMD) run $(CONTAINER_OPTS) --rm -v $(CURDIR):/go/src/github.com/liucxer/go-ceph$(VOLUME_FLAGS) $(RESULTS_VOLUME) $(CI_IMAGE_TAG)
test-multi-container: $(BUILDFILE) $(RESULTS_DIR)
	...
	$(CONTAINER_CMD) run --device /dev/fuse --cap-add SYS_ADMIN $(CONTAINER_OPTS) --rm \
		...
		-v $(CURDIR):/go/src/github.com/liucxer/go-ceph$(VOLUME_FLAGS) $(RESULTS_VOLUME) \
